...
#' 1. Optionally truncating outliers based on a specified threshold in units of standard deviations.
...
#' @param threshold Numeric or NULL. If specified, finite values higher than `threshold` are set to `threshold`
#'   and values lower than `-threshold` are set to `-threshold` (e.g., 4 to limit the series to [-4,4]). Default is NULL.
#'
#' @return Numeric vector with processed series, where Inf/-Inf are replaced and optional truncation is applied.
#'
#' @examples
#' # Process monthly series with truncation at +/-4 (i.e., 4 standard deviations)
#' process_inf(series, frequency = 12, threshold = 4)
#'
#' @export
process_inf <- function(series, frequency = 12, threshold = NULL) {
  ...
  if (!is.null(threshold)) {
    if (!is.numeric(threshold) || length(threshold) != 1 || threshold <= 0) {
      stop("`threshold` must be a single positive number or NULL.")
    }
  }
  ...
  if (!is.null(threshold)) {
    ...
    series[finite_idx & series > threshold]  <- threshold
    series[finite_idx & series < -threshold] <- -threshold
  }
